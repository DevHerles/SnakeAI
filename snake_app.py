...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, settings, show=True, fps=0.25):
        super().__init__()
        ...
        if self.settings["selection_type"].lower() == "plus":
            self._next_gen_size = (
                self.settings["num_parents"] + self.settings["num_offspring"]
            )
        elif self.settings["selection_type"].lower() == "comma":
            self._next_gen_size = self.settings["num_offspring"]
        else:
            raise Exception(
                'Selection type "{}" is invalid'.format(self.settings["selection_type"])
            )
    ...
    def init_window(self):
        ...
        self.nn_viz_window.setGeometry(
            QtCore.QRect(
                ...
                0,
                600,
                self._snake_widget_height + self.border[1] + self.border[3] + 200,
            )
        )
        ...
        self.snake_widget_window.setGeometry(
            QtCore.QRect(
                600 + self.border[0],
                self.border[1],
                self.snake_widget_width,
                self.snake_widget_height,
            )
        )
    ...
    def update(self) -> None:
        ...
        # Current individual is dead
            ...
            if (
                self.current_generation > 0
                and self._current_individual == self._next_gen_size
            ) or (
                self.current_generation == 0
                and self._current_individual == settings["num_parents"]
            ):
                print(self.settings)
                print(
                    "======================= Gneration {} =======================".format(
                ...
                current_pop = (
                    self.settings["num_parents"]
                    if self.current_generation == 0
                    else self._next_gen_size
                )
...
class GeneticAlgoWidget(QtWidgets.QWidget):
    def __init__(self, parent, settings):
        ...
        lifespan = (
            str(settings["lifespan"]) if settings["lifespan"] != np.inf else "infinite"
        )
        self._create_label_widget_in_grid(
        ...
        network_architecture = "[{}, {}, 4]".format(
            settings["vision_type"] * 3 + 4 + 4,
            ", ".join(
                [
                    str(num_neurons)
                    for num_neurons in settings["hidden_network_architecture"]
                ]
            ),
        )
        self._create_label_widget_in_grid(
    ...
    def _create_label_widget_in_grid(
        self,
        string_label: str,
        font: QtGui.QFont,
        grid: QtWidgets.QGridLayout,
...
class SnakeWidget(QtWidgets.QWidget):
    ...
    def draw_snake(self, painter: QtGui.QPainter) -> None:
        ...
        def _draw_line_to_apple(
            ...
            end_x = (
                drawable_vision.apple_location.x * SQUARE_SIZE[0] + SQUARE_SIZE[0] / 2
            )
            end_y = (
                drawable_vision.apple_location.y * SQUARE_SIZE[1] + SQUARE_SIZE[1] / 2
            )
            painter.drawLine(start_x, start_y, end_x, end_y)
            return end_x, end_y

        def _draw_line_to_self(
            painter: QtGui.QPainter,
            start_x: int,
            start_y: int,
            drawable_vision: DrawableVision,
            ...
            end_x = (
                drawable_vision.self_location.x * SQUARE_SIZE[0] + SQUARE_SIZE[0] / 2
            )
            end_y = (
                drawable_vision.self_location.y * SQUARE_SIZE[1] + SQUARE_SIZE[1] / 2
            )
            painter.drawLine(start_x, start_y, end_x, end_y)
            return end_x, end_y
        ...
        for point in self.snake.snake_array:
            painter.drawRect(
                point.x * SQUARE_SIZE[0],  # Upper left x-coord
                ...
                for drawable_vision in self.snake._drawable_vision:
                    ...
                    if drawable_vision.apple_location and drawable_vision.self_location:
                        apple_dist = self._calc_distance(
                            start.x,
                            drawable_vision.apple_location.x,
                            start.y,
                            drawable_vision.apple_location.y,
                        )
                        self_dist = self._calc_distance(
                            start.x,
                            drawable_vision.self_location.x,
                            start.y,
                    ...
                    if drawable_vision.wall_location:
                        painter.setPen(QtGui.QPen(Qt.black))
                        end_x = (
                            drawable_vision.wall_location.x * SQUARE_SIZE[0]
                            + SQUARE_SIZE[0] / 2
                        )
                        end_y = (
                            drawable_vision.wall_location.y * SQUARE_SIZE[1]
                            + SQUARE_SIZE[1] / 2
                        )
                        painter.drawLine(start_x, start_y, end_x, end_y)
    ...
    def draw_apple(self, painter: QtGui.QPainter) -> None:
        ...
        if apple_location:
            ...
            painter.drawRect(
                apple_location.x * SQUARE_SIZE[0],
...
def save_stats(population: Population, path_to_dir: str, fname: str):
    ...
    trackers = [("steps", frames), ("apples", apples), ("fitness", fitness)]
